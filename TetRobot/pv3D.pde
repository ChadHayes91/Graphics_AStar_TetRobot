...
// returns a new point rotated from 'start' clockwise around the axis
// formed with axisBase and axisDir to the specified angle (in radians).
// inspiration from the equation at the bottom of the section featured here:
// https://en.wikipedia.org/wiki/Rotation_matrix#Rotation_matrix_from_axis_and_angle
pt Rotation(pt start, vec axisDir, pt axisBase, float angle) {
  vec axisDirNorm = U(axisDir);
  pt rotationPt = P(axisBase, dot(V(axisBase, start), axisDirNorm), axisDirNorm);
  vec startVec = V(rotationPt, start);
  vec endVec = A(cross(V(cos(angle), cross(axisDirNorm, startVec)), axisDirNorm),
                 V(sin(angle), cross(axisDirNorm, startVec)));
  endVec = A(endVec, V(dot(axisDirNorm, startVec), axisDirNorm));
  return P(rotationPt, endVec);
}
...
float distance(pt A, pt B, boolean considerZ) {
    float ax = A.x;
    float ay = A.y;
    float az = A.z;
    float bx = B.x;
...
class obst
{
    ...
    void showObst() {
      ...
      float y = center.y;
      fill(red);
      caplet(P(x, y, 0), radius, P(x, y, h), radius);
      sphere(P(x, y, h), radius);
    }
    
    boolean ptInObst(pt P) {      
      float dist = distance(P, center, false);
      return dist <= radius;
    }
}
...
boolean isLegalMove(pt A, pt B, pt C) {
  for (int i = 0; i < n_obstacles; i++) {
    // Check if centers inside triangle
    if (ptInTriangle(A, B, C, obstacles[i].center)) return false;
    // Check if edges intersect obstacles
    if (circleIntersectsTriangle(A, B, C, obstacles[i].center, obstacles[i].radius)) return false;
  }
  
  return true;  
}

boolean circleIntersectsTriangle(pt A, pt B, pt C, pt center, float r) {
  if (lineIntersectsCircle(A, B, center, r) || lineIntersectsCircle(B, C, center, r) || lineIntersectsCircle(C, A, center, r)) { //<>//
    return true;
  }
  return false;
}

boolean lineIntersectsCircle(pt A, pt B, pt center, float r) {
  vec AB = U(V(A, B)); //<>//
  vec AC = V(A, center);
  float d = d(AB, AC);
  if (d > V(A, B).norm()) d = V(A, B).norm();
  if (d < 0) d = 0;
  pt proj = P(A, V(d, AB));
  
  float dist = distance(center, proj, false);
  return (dist <= r);
}

boolean rightTurn(pt A, pt B, pt C) {
  vec N = R(V(A, B), PI/2.0, V(0,0,1));
  return d(N, V(B, C)) > 0;
}


// If pt P is inside triangle ABC
boolean ptInTriangle(pt A, pt B, pt C, pt P) {
  return (rightTurn(A, B, P) == rightTurn(B, C, P)) && (rightTurn(B, C, P) == rightTurn(C, A, P));
}
